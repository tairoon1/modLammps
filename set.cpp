...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      if (!atom->vfrac_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      if (dvalue < 0.0 && dvalue > 1.0) error->all(FLERR,"Invalid lambda in set command");
      set(LAMBDA);
      iarg += 2;
      ...
      if (!atom->vfrac_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      if (dvalue < 0.0) error->all(FLERR,"Invalid temperature in set command");
      set(TEMPERATURE);
      iarg += 2;
      ...
      if (!atom->vfrac_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      if (dvalue < 0.0) error->all(FLERR,"Invalid chemPotential in set command");
      set(CHEMPOTENTIAL);
      iarg += 2;
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    // set value for custom integer or double vector
      ...
      if (dvalue < 0.0 && dvalue > 1.0) error->one(FLERR,"Invalid lambda in set command");
